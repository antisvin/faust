                                  ...
                                  const string& ccs)
{
    if (delay < gGlobal->gMaxCopyDelay) {
        ...
        // -- compute the new samples
        fClass->addExecCode(OldStatement(subst("$0[i] = $1;", dlname, cexp)));
        ...
        // -- update index
        fClass->addPreCode(OldStatement(subst("$0 = ($0+$1)&$2;", idx, idx_save, mask)));

        // -- compute the new samples
        fClass->addExecCode(OldStatement(subst("$0[($2+i)&$3] = $1;", dlname, cexp, idx, mask)));

        // -- save index
        fClass->addPostCode(OldStatement(subst("$0 = count;", idx_save)));
    }
}
